...
namespace ModelLayer.Helper
{
    public class ComparerForModificationEntryCreation
    {
            ...
            out List<ModificationEntry> listWithCreation, out List<ModificationEntry> listWithDeletion)
        {
            ...
            {
                listCreation.Add(GetNewModificationEntry(newContact.History.Count.ToString(), oldContact.History.Count.ToString(), newContact.Id, MODEL_TYPE.CONTACT, DATA_TYPE.HISTORY_ELEMENT, userOfModification, MODIFICATION.MODIFIED));
            }
            ...
            GetModificationsForContactPossibilitiesObject(oldContact.ContactPossibilities, newContact.ContactPossibilities, newContact.Id, userOfModification, deleteEntries, listCreation, listDeletion);
            ...
            out List<ModificationEntry> listWithCreation, out List<ModificationEntry> listWithDeletion)
        {
            ...
            {
                listCreation.Add(GetNewModificationEntry(newOrga.OrganizationContacts.Count.ToString(), oldOrga.OrganizationContacts.Count.ToString(), newOrga.Id, MODEL_TYPE.ORGANIZATION, DATA_TYPE.EMPLOYEES, userOfModification, MODIFICATION.MODIFIED));
            }
        //    if (oldOrga.History.Count != newOrga.History.Count)
         //   {
         //       listCreation.Add(GetNewModificationEntry(newOrga.History.Count.ToString(), oldOrga.History.Count.ToString(), newOrga.Id, MODEL_TYPE.ORGANIZATION, DATA_TYPE.HISTORY_ELEMENT, userOfModification, MODIFICATION.MODIFIED));
         //   }
            listCreation.AddRange(GetModificationsForAddressObject(oldOrga.Address, newOrga.Address, newOrga.Id, userOfModification));
            GetModificationsForContactPossibilitiesObject(oldOrga.Contact, newOrga.Contact, newOrga.Id, userOfModification, deleteEntries, listCreation, listDeletion);
            listWithCreation = listCreation;
            listWithDeletion = listDeletion;
        }
        ...
        private static void GetModificationsForContactPossibilitiesObject(ContactPossibilities oldOne, ContactPossibilities newOne, long idOfModel, string userOfModification, bool deleteEntries,
            List<ModificationEntry> listWithCreation, List<ModificationEntry> listWithDeletion)
        {
            if (!oldOne.Fax.Equals(newOne.Fax))
            {
                if (string.IsNullOrEmpty(newOne.Fax) && deleteEntries)
                {
                    listWithDeletion.Add(GetNewModificationEntry(newOne.Fax, oldOne.Fax, idOfModel, MODEL_TYPE.CONTACT, DATA_TYPE.FAX, userOfModification, MODIFICATION.MODIFIED, -1, true));
                } else
                {
                    listWithCreation.Add(GetNewModificationEntry(newOne.Fax, oldOne.Fax, idOfModel, MODEL_TYPE.CONTACT, DATA_TYPE.FAX, userOfModification, MODIFICATION.MODIFIED));
                }                
            }
            if (!oldOne.Mail.Equals(newOne.Mail))
            {
                ...
                {
                    listWithDeletion.Add(GetNewModificationEntry(newOne.Mail, oldOne.Mail, idOfModel, MODEL_TYPE.CONTACT, DATA_TYPE.MAIL, userOfModification, MODIFICATION.MODIFIED, -1, true));
                }
                else
                {
                    listWithCreation.Add(GetNewModificationEntry(newOne.Mail, oldOne.Mail, idOfModel, MODEL_TYPE.CONTACT, DATA_TYPE.MAIL, userOfModification, MODIFICATION.MODIFIED));
                }
            }
            if (!oldOne.PhoneNumber.Equals(newOne.PhoneNumber))
            {
                ...
                {
                    listWithDeletion.Add(GetNewModificationEntry(newOne.PhoneNumber, oldOne.PhoneNumber, idOfModel, MODEL_TYPE.CONTACT, DATA_TYPE.PHONE, userOfModification, MODIFICATION.MODIFIED, -1, true));
                }
                else
                {
                    listWithCreation.Add(GetNewModificationEntry(newOne.PhoneNumber, oldOne.PhoneNumber, idOfModel, MODEL_TYPE.CONTACT, DATA_TYPE.PHONE, userOfModification, MODIFICATION.MODIFIED));
                }
            }
            GetModificationsForContactEntriesObject(oldOne.ContactEntries, newOne.ContactEntries, idOfModel, userOfModification, deleteEntries, listWithCreation, listWithDeletion);
        }
            ...
            List<ModificationEntry> listWithCreation, List<ModificationEntry> listWithDeletion)
        {
            ...
            foreach (ContactPossibilitiesEntry entryOld in oldOne)
            {
                ...
                {                    
                    listWithDeletion.Add(GetNewModificationEntry("" , entryOld.ContactEntryValue, idOfModel, MODEL_TYPE.CONTACT, dataType, userOfModification, MODIFICATION.MODIFIED, idx, true));
                }
                else 
                {
                    if (newEntryToFind == null || !newEntryToFind.ContactEntryName.Equals(entryOld.ContactEntryName) || !newEntryToFind.ContactEntryValue.Equals(entryOld.ContactEntryValue))
                    {
                        ...
                        {
                            listWithCreation.Add(GetNewModificationEntry(newName, entryOld.ContactEntryName, idOfModel, MODEL_TYPE.CONTACT, dataType, userOfModification, MODIFICATION.MODIFIED, idx));
                        }
                        if (!newValue.Equals(entryOld.ContactEntryValue))
                        {
                            listWithCreation.Add(GetNewModificationEntry(newValue, entryOld.ContactEntryValue, idOfModel, MODEL_TYPE.CONTACT, dataType, userOfModification, MODIFICATION.MODIFIED, idx));
                        }
                    }                    
                }
                idx++;
